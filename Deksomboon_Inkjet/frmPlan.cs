...
namespace Deksomboon_Inkjet
{
    public partial class frmPlan : Form
    {
        ...
        private void InitializeTimer_send_text()
        {
            timer4 = new Timer();
            timer4.Interval = 10000; // 10 วินาที
            timer4.Tick += new EventHandler(OnTimerTick_send_text);
            timer4.Start();
        }
        ...
        private async void OnTimerTick_send_text(object sender, EventArgs e)
        {
            command_send_text();
        }
        ...
        public void open_line_setting()
        {
            ...
            if (!string.IsNullOrEmpty(emp_id))
            {
                ...
                if (frmSettingInstance.ShowDialog() == DialogResult.OK)
                {
                    ...
                    {
                        DataLog.Add_Authorized_Log(0, Int32.Parse(emp_id), start_date, "เปลี่ยนไลน์ผลิต/เครื่องพิมพ์", 1, txtTenDigit_temp.Text);
                    }
                    else
                    {
                        DataLog.Add_Authorized_Log(Int32.Parse(txtOrdID.Text), Int32.Parse(emp_id), start_date, "เปลี่ยนไลน์ผลิต/เครื่องพิมพ์", 1, txtTenDigit_temp.Text);
                    }                   
                        refreash_order();
                    clear_authroized();
                }
        ...
        public void get_order()
        {
            ...
            if (table.Columns.Count == 0)
            {
                ...
                table.Columns.Add("สถานะผลิต", typeof(string));  
        ...
        public void get_order_detail_first_row()
        {
            if (table.Rows.Count > 0) // ตรวจสอบว่า DataTable (table) มีข้อมูลหรือไม่
            {
                ...
                if (selectedRow != null)
                {
                    ...
                    string columnBatch = selectedRow["Batch"].ToString();
            ...
            //OrderGrid.Columns["Batch"].Visible = false;
        ...
        private async void StartButton_Click(object sender, EventArgs e)
        {
            ...
            if (status[0] == "Jet Running")
            {
                // Commented code for potential future use
                // if (status[1] == "Waiting" || status[1] == "Printing")
                // {
                string emp_id = Authorized.authorized_level_1(txtEmployeeCode.Text, txtEmployeepass.Text);
                ...
                if (!string.IsNullOrEmpty(emp_id))
                {
                    DialogResult confirmStartPrint = MessageBox.Show(
                        $"คุณต้องการเริ่มผลิต Formula : {txtFormula.Text} , batch : {txtBatch.Text} หรือไม่",
                        "คุณต้องการเริ่มต้นการผลิตหรือไม่ ?",
                        MessageBoxButtons.YesNo,
                        MessageBoxIcon.Question);

                    if (confirmStartPrint == DialogResult.Yes)
                    {
                        btnEditOrderPosition.Visible = false;
                        tableLayoutOrderPosition.Visible = false;

                        bool startPrint = await command_start_print();
                        StartButton.Enabled = false;
                        ...
                        if (startPrint)
                        {
                            ...
                            DateTime st = DateTime.Now.AddYears(-543);
                            string startDate = st.AddSeconds(-st.Second).ToString();
                            txtOrderDateStart.Text = startDate;

                            string ordStatusPrint = "กำลังผลิต";

                            Order.Update_Order_Status(txtOrdID.Text, txtBatch.Text, "กำลังผลิต", ordStatusPrint, Int32.Parse(txtCount.Text));
                            DataLog.Add_DatLog(Int32.Parse(txtOrdID.Text), txtTenDigit_temp.Text, 0, startDate, "", Int32.Parse(emp_id));
                            DataLog.Add_Authorized_Log(Int32.Parse(txtOrdID.Text), Int32.Parse(emp_id), startDate, "กำลังผลิต", 1, txtTenDigit_temp.Text);

                            refreash_order();
                            clear_authroized();

                            bool checkSendText = await command_send_text();
                            if (checkSendText)
                            {
                                MessageBox.Show("ส่งข้อความไปที่เครื่องอินเจ็กเรียบร้อยแล้ว", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                InitializeTimer_send_text();
                                refreash_order();
                                clear_authroized();
                            }
                            else
                        ...
                        {
                            MessageBox.Show("Start Jet ไม่สำเร็จ", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                }
                // }
            ...
            {
                DialogResult confirmStartJet = MessageBox.Show(
                    "อินเจ็กของคุณยังไม่เริ่มทำงาน คุณต้องการส่งคำสั่งไปเริ่มอินเจ็กหรือไม่ ?",
                    "Confirm Start Jet",
                    MessageBoxButtons.YesNo,
                    MessageBoxIcon.Question);

                if (confirmStartJet == DialogResult.Yes)
                {
                    bool jetStart = await command_start_jet();
                    if (jetStart)
                    {
                        MessageBox.Show("กำลังเริ่มอินเจ็กกรุณารอสักครู่", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
                        MessageBox.Show("ไม่สามารถส่งคำสั่ง start jet โปรดตรวจสอบอินเจ็กว่าพร้อมใช้งานหรือไม่", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                    ...
                    //{
                        ...
                        //{
                            ...
                            int sum_count = Int32.Parse(txtCountEnd.Text) - Int32.Parse(txtCount2.Text);
                                        ...
                                        string ord_status_print = "ผลิตไม่ครบ";
                            ...
                            txtCount.Text = sum_currunt_count.ToString();
                                    ...
                                    Order.Update_Order_Status(ord_id, batch, "หยุดผลิตชั่วคราว" , ord_status_print , sum_currunt_count);
                                        ...
                                        {
                                            DataLog.Update_DateLog(Int32.Parse(ord_id), sum_count, end_date, Int32.Parse(emp_id), txtTenDigit_temp.Text, txtOrderDateStart.Text);
                                        }
                                        DataLog.Add_Authorized_Log(Int32.Parse(ord_id), Int32.Parse(emp_id), end_date, "หยุดผลิตชั่วคราว", 1, txtTenDigit_temp.Text);
                            txtCountEnd.Text = "";
                            sim_count = 1;
                            refreash_order();
                                    clear_authroized();
        ...
        private async void EndBatchButton_Click(object sender, EventArgs e)
        {
            ...
            using (ChangeBatch frm = new ChangeBatch(
                obj, form, txtEmployeeCode.Text, txtEmployeepass.Text, dif_count, emp_id,
                txtOrderDateStart.Text, txtAmount.Text, txtCount.Text))
            {
                if (frm.ShowDialog() == DialogResult.OK)
                {
                    MessageBox.Show("เปลี่ยน batch เรียบร้อยแล้ว");
        ...
        private async void EmegencyButton_Click(object sender, EventArgs e)
        {
            ...
            if (isConnected == true)
            {
                ...
                if (!string.IsNullOrEmpty(emp_id))
                {
                    ...
                    using (AddEmegencyOrder frm = new AddEmegencyOrder(obj, form, txtEmployeeCode.Text, txtEmployeepass.Text, 0, emp_id, txtOrderDateStart.Text , txtAmount.Text))
                    {
                        if (frm.ShowDialog() == DialogResult.OK)
                        {
                            refreash_order();
                    ...
                    //{
                        ...
                        if (!string.IsNullOrEmpty(emp_id))
                        {
                            ...
                            if (confrim == DialogResult.Yes)
                            {
                                ...
                                string ord_status_print = "ผลิตเสร็จ";
                        ...
                        DataRow selectedRow = table.Rows[0];
                                ...
                                DataLog.Update_DateLog(Int32.Parse(ord_id), sum_count, end_date, Int32.Parse(emp_id), txtTenDigit_temp.Text, txtOrderDateStart.Text);
                                DataLog.Add_Authorized_Log(Int32.Parse(ord_id), Int32.Parse(emp_id), end_date, "จบออร์เดอร์", 1, txtTenDigit_temp.Text);
                                txtCountEnd.Text = "";
                                sim_count = 1;
                                refreash_order();
                                clear_authroized();
                        ...
                        //{
                            ...
                            if (!string.IsNullOrEmpty(emp_id))
                            {
                                ...
                                Order.Update_Order_Status(txtOrdID.Text, txtBatch.Text, "ทดสอบพิมพ์" , "รอผลิต" , Int32.Parse(txtCount.Text));
                                        ...
                                        DataLog.Add_Authorized_Log(Int32.Parse(txtOrdID.Text), Int32.Parse(emp_id), start_date, "ทดสอบพิมพ์", 1, txtTenDigit_temp.Text);
                                        refreash_order();
                                clear_authroized();
